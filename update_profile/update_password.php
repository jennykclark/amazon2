...
<form class="style_form" id="update-password" onsubmit="return false">

<label for="email"><?=$_TXT[65]?></label>
      <input name="email" type="text" id="emailID" placeholder="Email" value="<?php echo $user['email'] ?>">
    </div>
    <!-- <div>
    ...
    <div>
    <label for="password"><?=$_TXT[71]?></label>
      <input name="password" id="passID" placeholder="password" type="password">
    </div>
      <input name="user_id" placeholder="user_id" type="hidden" value="<?php echo $_SESSION['user']['user_id'] ?>">
    </div>
<br>

  <button onclick="update_pass()" id="updateButton"><?=$_TXT[60]?></button>  
