...
$q->bindValue(":userID", $_SESSION['user']['user_id']);
...
<section id="UploadDIv">
    <h1><?=$_TXT[52]?></h1>
<form class="style_form" id="upload_item" onsubmit="return upload_item_form(); false" enctype="multipart/form-data">

<div>
  <label for="ItemName"><?=$_TXT[53]?></label>
  <input name="item_name" id="item_name_id" type="text" placeholder="<?=$_TXT[53]?>">
</div>
<div>
  <label for="itemPrice"><?=$_TXT[54]?></label>
  <input name="item_price" id="priceID" type="number" placeholder="<?=$_TXT[54]?> (DKK)">
</div>
<div>
  <label for="itemInstock"><?=$_TXT[55]?></label>
  <input name="item_stock" id="stockID" type="number" placeholder="<?=$_TXT[55]?>">
</div>
<br>
<div>
    <label for="description"><?=$_TXT[56]?></label>
    <textarea name="description" id="descrID" cols="40" rows="5" placeholder="<?=$_TXT[56]?>"></textarea>
</div>
<div>
  <label for="ItemImg"><?=$_TXT[57]?></label>
  <!-- <input name="item_img" type="text" placeholder="Image"> -->
  <input type="file" name="image">
</div>
<div>
      <input name="user_id" type="hidden" value="<?php echo $_SESSION['user']['user_id'];?>">
</div>
  
  <button id="uploadButton" onclick="upload()"><?=$_TXT[58]?></button>
